...
definePageMeta({
  title: 'Create Booking Package Page',
  layout: 'dashboard'
})
...
const schema = yup.object({
  ...
  entertainment_package_id: yup.string().required('Entertainment package Id is required'),
...
const getPrice = async () => {
  if(entertainment_package_id.value != undefined || qty.value != undefined) {
    await bundleStore.getBundleById(entertainment_package_id.value)
    setValues({
      total_price: bundleStore.bundle.price * qty.value,
      total_pay: 0,
      total_change: 0,
    })
  } else {
        ...
        <div class="card-default">
          <form @submit.prevent="create" class="form w-100">
            <div class="row g-3">
              ...
              <div class="col-md-4">
                <div class="input-group d-flex flex-column">
                  <label for="details">Package</label>
                  <select v-model="entertainment_package_id" id="entertainment_package_id" class="input w-100" @change="getPrice({ entertainment_package_id: entertainment_package_id.value })">
                    <option v-for="(bundle, index) in bundleStore.bundleAll" :key="index" :value="bundle.id">{{bundle.name}}</option>
                  </select>
                  <p v-if="entertainmentPackageIdError" class="invalid-label">{{ entertainmentPackageIdError }}</p>
                </div>
              </div>
              <div class="col-md-4">
                <div class="input-group d-flex flex-column">
              ...
              <div class="col-md-4">
                <div class="input-group d-flex flex-column">
                  ...
                  <input type="number" class="input w-100" name="total_price" id="total_price"
                         placeholder="Select package first.." autocomplete="off" v-model="total_price" readonly>

    ...
    <div class="row align-items-center w-100">
      <div class="col-lg-5 d-none d-lg-inline-block">
        <div class="wrapper-banner position-relative">
          ...
          <img :src="bundleStore.bundle.image_path != null ? `http://localhost:8000/${bundleStore.bundle.image_path}` : imageNotFound" alt="Banner Image" class="img-fluid position-relative banner-image">
        </div>
      </div>
      <div class="offset-lg-1 col-lg-6">
        <h2 class="title mb-2">{{bundleStore.bundle.name}}</h2>
        ...
        <div class="button-group d-flex gap-2 align-items-center">
          ...
          <NuxtLink v-if="authStore.isLoggedIn && authStore.user.user.role != 'instructor'" :to="{path: `/homepage/booking/package/${bundleStore.bundle.id}`}" class="button-reverse">Booking Now</NuxtLink>
        </div>
      </div>
    </div>
  </section>
      ...
      <div class="row row-cols-2 row-cols-md-4 row-cols-xl-5 g-3 g-md-4 content-gap content-value">
        <div v-for="(service, index) in bundleStore.bundle.services" :key="index" class="col">
          <div class="card-service">
            <img :src="service.entertainment_service.image_path != null ? `http://localhost:8000/${service.entertainment_service.image_path}` : imageNotFound" alt="">
